    ...
    * method
    ...
    def fit_transform(self, X, _y=None, skip_num_points=0, skip_iter=0, init_for_skip=None):
        ...
        t = FuncThread(self.C.tsne_run_double,
                       ...
                       self.verbose, self.early_exaggeration, self.learning_rate,
...
class IncrementalMulticoreTSNE:
    """
    Compute t-SNE embedding using Barnes-Hut optimization and
    multiple cores (if avaialble).
    
    Compute position of all points based on the position of old points.
    ...
    * method
    ...
    def __init__(self,
        ...
        self.ffi = cffi.FFI()
        self.ffi.cdef(
            """void incremental_tsne_run_double(double* X, int N, int D, double* Y,
                                    int no_dims, double perplexity, double theta,
                                    int num_threads, int max_iter, int random_state,
                                    int verbose,
                                    double early_exaggeration, double learning_rate,
                                    double *final_error, int distance, int old_num_points, int join_times);""")
    ...
    def fit_transform(self, X, _y=None, old_num_points=0, join_times=1, init_for_old=None):
        assert X.ndim == 2, 'X should be 2D array.'
        ...
        assert old_num_points >= 0, 'old_num_points >= 0.'
        assert old_num_points < N, 'old_num_points <= ' + str(N) + '.'
        assert join_times >= 1, 'join_times >= 1.'
        assert join_times <= self.n_iter, 'join_times < ' + str(self.n_iter) + '.'
        assert join_times <= N - old_num_points, 'join_times < ' + str(N - old_num_points) + '.'

        if old_num_points > 0:
            assert isinstance(init_for_old, np.ndarray), 'init_for_old should be 2D array.'
            assert init_for_old.shape == (old_num_points, self.n_components), 'init_for_old\'s shape should be ' + str((old_num_points, self.n_components))
        ...
        if init_from_Y:
            ...
            Y = np.dot(Y, eig_vec)
        if old_num_points > 0:
            Y[0:old_num_points,:] = init_for_old
        ...
        t = FuncThread(self.C.incremental_tsne_run_double,
                       cffi_X, N, D,
                       cffi_Y, self.n_components,
                       self.perplexity, self.angle, self.n_jobs, self.n_iter, self.random_state,
                       self.verbose, self.early_exaggeration, self.learning_rate,
                       cffi_final_error, int(self.cheat_metric), old_num_points, join_times)
        t.daemon = True
        t.start()
